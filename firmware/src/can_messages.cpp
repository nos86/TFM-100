 ...
 *
 * Each `buildPayload()` returns a pointer to a static buffer containing the
 * payload and writes its length to `len`. Static buffers are safe because the
 * `J1939Manager` copies payloads for messages that return true from
 * `shallPayloadCopied()`.
 *
 ...
 *   with rounding and clamping to avoid overflow and preserve accuracy.
 */
...
uint8_t *HeartbeatMessage::buildPayload(uint8_t *len)
{
   ...
   * - byte 3: reserved
   * - bytes 4..7: 32-bit uptime (little-endian, LSB at byte 4)
   *
   * Lifetime: static buffer; safe because manager copies the payload when
...
uint8_t *CAN_FilteredTemperatureAndFlow::buildPayload(uint8_t *len)
{
   ...
   * Lifetime: static buffer; safe because manager copies payloads when
   * `shallPayloadCopied()` returns true.
   */
