...
void setup()
{
  ...
  if ((mcp_init = (CAN0.begin(MCP_ANY, CAN_250KBPS, MCP_8MHZ) == CAN_OK)))
    ...
    {
      // Accept all VSCP Level 1 extended frames for nickname discovery and DM.
      // The VSCP core filters relevant events internally.
      // RXB0: accept all extended frames
      CAN0.init_Mask(0, 1, 0x00000000);
      CAN0.init_Filt(0, 1, 0x00000000);
      // RXB1: accept all extended frames
      CAN0.init_Mask(1, 1, 0x00000000);
      CAN0.init_Filt(2, 1, 0x00000000);
  ...
  // Initialise VSCP node: nickname discovery + decision matrix
  vscp_node_setup(node_id);
...
void loop()
{
  ...
  // Drive VSCP protocol stack: nickname discovery, DM, incoming frame dispatch
  vscp_node_process();
